...
void write_cose(std::istream&is, std::ostream&os, std::vector<uint8_t>& stream_decompresso, uint32_t match_distance, uint32_t literal_length, uint32_t match_length)
{
	...
	uint32_t start = stream_decompresso.size() - match_distance;
...
void lzvn_decode(std::istream& is, std::ostream& os) {
	...
	while (1)
	{
		...
		switch (opcode)
		{
		break; case 0b11110000: operation = "large match"; log<<std::bitset<8>(opcode) <<" "<<operation<<'\n';
		break; case 0b11100000:operation = "large literal"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00001110: operation = "nop"; log << std::bitset<8>(opcode)  << " " << operation << '\n'; continue;
		break; case 0b00010110: operation = "nop"; log << std::bitset<8>(opcode)  << " " << operation << '\n'; continue;
		break; case 0b00000110: operation = "eos"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00011110: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00100110: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00101110: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00110110: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b00111110: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		default:
			break;
		}
		if (operation=="large match")
		{
			raw_read(is, match_length, 1);
			match_length += 16;
			write_cose(is, os, stream_decompresso, match_distance, 0, match_length);
			continue;
		}
		else if (operation == "large literal") {
			raw_read(is, literal_length, 1);
			literal_length += 16;
		...
		{
		break; case 0b1111: operation = "small match"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b1110: operation = "small literal"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b0111: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		break; case 0b1101: operation = "udef"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		default:
			break;
		}
		...
		if (distance_match==0b101)
		{
			...
			operation = "medium distance"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		...
		if (distance_match==0b111)
		{
			...
			operation = "large distance"; log << std::bitset<8>(opcode) << " " << operation << '\n';
			...
			match_length = (opcode >> 3) & 0b111;
			match_length += 3;
			operation = "previous distance"; log << std::bitset<8>(opcode) << " " << operation << '\n';
		...
		operation = "small distance"; log << std::bitset<8>(opcode)  << " " << operation << '\n';
		literal_length = (opcode >> 6) & 0b11;
		match_length = ((opcode >> 3) & 0b111);
		match_length += 3;
